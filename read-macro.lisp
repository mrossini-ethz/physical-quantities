...
(defmethod eval-quantity ((value number) (error number) unit-a (unit-b (eql nil)))
  (make-quantity :value value :error error :unit unit-a))
;; #q(<n> [+/- <n>] m / s -> km / h)
(defmethod eval-quantity ((value number) (error number) unit-a unit-b)
  (make-quantity :value (convert-units value unit-a unit-b) :error (if (minusp error) error (convert-units error unit-a unit-b)) :unit unit-b))
;; #q(<q>)
